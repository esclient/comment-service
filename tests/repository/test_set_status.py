...
@pytest.mark.asyncio
async def test_set_status_returns_true_on_update(mocker: MockerFixture, faker: Faker) -> None:
    conn = mocker.Mock()
...
@pytest.mark.asyncio
async def test_set_status_returns_false_on_exception(mocker: MockerFixture, faker: Faker) -> None:
    pool = mocker.Mock()
    pool.acquire.side_effect = RuntimeError("boom")

    result = await set_status(pool, faker.random_int(min=1, max=100000), "HIDDEN")
