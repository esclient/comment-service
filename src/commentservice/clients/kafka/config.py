...
class KafkaConfig:
    def __init__(self) -> None:
        ...
        self.consumer_group_id = os.getenv(
            "KAFKA_CONSUMER_GROUP_ID", "comment-service-group"
        )
        self.max_retries = int(os.getenv("KAFKA_MAX_RETRIES", "3"))
        self.retry_backoff_ms = int(
            os.getenv("KAFKA_RETRY_BACKOFF_MS", "1000")
        )
        self.enable_ssl = (
            os.getenv("KAFKA_ENABLE_SSL", "false").lower() == "true"
        )
